...
check('后端 BloodTest Model 字段规范', () => {
  ...
  const optionalFields = ['neu?', 'lym?', 'crp?', 'notes?', 'isAbnormal', 'chemoCycleId?'];
...
check('Validation 中间件字段对齐 Model', () => {
  ...
  const expectedFields = ['date', 'wbc', 'rbc', 'hgb', 'plt', 'neu', 'lym', 'crp', 'notes', 'chemoCycleId'];
...
check('前端 types/index.ts 字段规范', () => {
  ...
  const contractFields = [
    ...
    'neu?:', 'lym?:', 'crp?:', 'notes?:', 'isAbnormal?:', 'chemoCycleId?:', 'createdAt?:', 'updatedAt?:',
  ];
